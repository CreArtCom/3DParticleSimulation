...
public abstract class Simulation extends Max
{
	...
	/** Blob's position message : ~ blob_index x y z */
	...
	/** Blob's sphere brush message : ~ blob_index radius */
	protected static String MSG_BLOB_SPHERE = "blob_sphere";
	
	/** Blob's box brush message : ~ blob_index width height depth */
	protected static String MSG_BLOB_BOX = "blob_box";
    ...
    protected boolean TreatMessage(String message, Atom[] args)
    {
		...
		{			
			...
			else if(message.contentEquals(MSG_BLOB_SPHERE))
				blobsSystem.setSphereBrush(args[0].toInt(), args[1].toDouble());
		...
		{
			if(message.contentEquals(MSG_BLOB_BOX))
				blobsSystem.setBoxBrush(args[0].toInt(), args[1].toDouble(), args[2].toDouble(), args[3].toDouble());
			
			else if(message.contentEquals(MSG_BLOB_POSITION))
				applyBlob(args[0].toInt(), new Vector3(args[1].toDouble(), args[2].toDouble(), args[3].toDouble()));
	 ...
	 */
	protected abstract void applyBlob(int index, Vector3 position);
