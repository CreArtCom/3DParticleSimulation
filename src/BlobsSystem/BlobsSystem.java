...
{
	protected static final int SPHEREBRUSH = 0;
	protected static final int BOXBRUSH = 1;
     ...
     * @since	1.0
     */
	public Map<Blob, List<Vector3[]>> getBlobsMouvements()
	{
		Map<Blob, List<Vector3[]>> result = new HashMap<Blob, List<Vector3[]>>();
	...
	public void setBoxBrush(int index, double width, double height, double depth)
	{
		for(Blob blob : getBlobs(index))
		{
			Brush brush = blob.getBrush();

			if(brush instanceof BoxBrush) {
				((BoxBrush) brush).setWidth(width);
				((BoxBrush) brush).setHeight(height);
				((BoxBrush) brush).setDepth(depth);
			}
			else
				simulation.printOut("Blob n°" + index + " has no a Box Brush.");
		}
	}
	
	public void setSphereBrush(int index, double radius)
	{
		for(Blob blob : getBlobs(index))
		{
			Brush brush = blob.getBrush();
			...
			if(brush instanceof SphereBrush)
				((SphereBrush) brush).setRadius(radius);
			else
				simulation.printOut("Blob n°" + index + " has no a Sphere Brush.");
		}
	}
	...
	public void setBrush(int index, int brushIndex)
	{
		for(Blob blob : getBlobs(index))
		{
			...
			{
				case SPHEREBRUSH:
					blob.setBrush(new SphereBrush());
					break;

				case BOXBRUSH:
					blob.setBrush(new BoxBrush());
					break;
	...
	public Blob setPosition(int index, Vector3 position)
	{
		Blob blob = null;
